...
nom_count_dict = {}
for year, nominators in nominated.items():
...
for year, winnerList in winners.items():
    for winner in winnerList:
        # get returns 0 if winner is not in the dictionary, then increment director count by one
        win_count_dict[winner] = win_count_dict.get(winner, 0) + 1


# Q2: Provide a list with the name(s) of the director(s) with the most Oscar wins. We are asking for a list because
# there could be more than 1 director tied for the most Oscar wins.
win_count_dict = {}
for year, winnerList in winners.items():
    for winner in winnerList:
        # get returns 0 if winner is not in the dictionary, then increment director count by one
...
# Second part
highest_count = 0   # keep track of the highest count of wins
most_win_director = []

for key, value in win_count_dict.items():
    if value > highest_count:
        highest_count = value   # replace the value as the highest_count if value > highest_count
