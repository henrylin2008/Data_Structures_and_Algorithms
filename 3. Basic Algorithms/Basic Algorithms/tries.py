...
print('Is "a"   a word: {}'.format(basic_trie['a']['word_end']))        # True
print('Is "ad"  a word: {}'.format(basic_trie['a']['d']['word_end']))   # False
print('Is "add" a word: {}'.format(basic_trie['a']['d']['d']['word_end']))  # True

# You can lookup a word by checking if word_end is True after traversing all the characters in the word. Let's look
# at the word "hi". The first letter is "h", so you would call basic_trie['h']. The second letter is "i",
# so you would call basic_trie['h']['i']. Since there's no more letters left, you would see if this is a valid word
# by getting the value of word_end. Now you have basic_trie['h']['i']['word_end'] with True or False if the word
# exists.

# In basic_trie, words "a" and "add" overlapp. This is where a Trie saves memory. Instead of having "a" and "add" in
# different cells, their characters treated like nodes in a tree. Let's see how we would check if a word exists in
# basic_trie.
...
# whether it's characters, words, or network nodes. When working on a problem with sequence data, ask yourself if a Trie
