# The Tower of Hanoi is a puzzle where we have three rods and n unique sized disks. The three rods are - source,
# destination, and auxiliary as shown in the figure below.
#
# Initially, all the n disks are present on the source rod. The final objective of the puzzle is to move all disks
# from the source rod to the destination rod using the auxiliary rod.
#
...
# You will be given the number of disks num_disks as the input parameter. Write a recursive function tower_of_Hanoi()
# that prints the "move" steps in order to move num_disks number of disks from Source to Destination using the help
# of Auxiliary rod.
...
#
# An example illustration for num_disks = 4 can be visualized in this GIF from
...
#
# 3.For num_disks > 1, just think of your FIRST set of steps. You want to pick the bottom most disk on the source,
# to be transferred to the destination. For this reason, you will perform the steps below:
#    - Step 1: Move num_disks - 1 from source to auxiliary
#    - Step 2: Now you are left with only the largest disk at source. Move the only leftover disk from source to
#              destination
